document.addEventListener("DOMContentLoaded", () => {
  ...
  function createDayElement(date, isOtherMonth) {
    ...
    // Obtener medicamentos para este día
    const medicamentosDia = getMedicamentosPorDia(date)

    // Mostrar vista previa de medicamentos (máximo 2)
    const maxPreview = 2
    medicamentosDia.slice(0, maxPreview).forEach((med) => {
      const preview = document.createElement("div")
      preview.className = "medicamento-preview"
      preview.textContent = `${med.hora} - ${med.nombre}`
      diaElement.appendChild(preview)
    })

    // Añadir indicador de cantidad si hay más de 2 medicamentos
    if (medicamentosDia.length > maxPreview) {
      const indicador = document.createElement("div")
      indicador.className = "indicador-medicamentos"
      indicador.textContent = medicamentosDia.length
    ...
    // Añadir atributo de fecha para identificar el día
    diaElement.setAttribute("data-date", date.toISOString().split("T")[0])
